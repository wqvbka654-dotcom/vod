...
class Spider(Spider):
    ...
    def homeContent(self, filter):
        ...
        di = '{"recSwitch":true,"pageFlag":"","theaterSubscriptSwitch":false}'
        ...
        if response.status_code == 200:
            ...
            js = detail['channelGroupData'][0]['channelData']

            for vod in js:

                channelId = vod['channelId']

                channelName = vod['channelName']

                type_id = str(channelId) + "@" + str(channelName)

                result["class"].append({"type_id": type_id, "type_name":  channelName})
    ...
    def categoryContent(self, cid, pg, filter, ext):
        ...
        di = f'{{"recSwitch":true,"storePageId":10002,"channelGroupId":"10","channelId":"{fenge[0]}","channelName":"{fenge[1]}","pageFlag":{str(page)},"theaterSubscriptSwitch":true}}'
        detail = self.decrypt_wb(di)
        url = f"{xurl}/free-video-portal/portal/1125"
        response = requests.post(url=url, headers=headerx, data=detail)
    ...
    def detailContent(self, ids):
        ...
        if response.status_code == 200:
            ...
            url = 'https://4907.kstore.space/GF/%E6%B2%B3%E9%A9%AC.txt'
