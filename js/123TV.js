...
async function category(tid, pg, filter, extend) {
    let p = parseInt(pg) || 1;
    let resp = await req(`${host}/t/${tid}/p${p}.html`, {
        headers
    });
    return JSON.stringify({
        list: getList(resp.content),
...
async function detail(id) {
    ...
    // 解析播放线路和集数（线路名改为冉神1、冉神2、冉神3...排序）
    const ppMatch = html.match(/var\s+pp\s*=\s*({[\s\S]*?});/);
    if (ppMatch) {
        const {
            la = []
        } = JSON.parse(ppMatch[1]);
...
async function search(wd, quick, pg) {
    let p = parseInt(pg) || 1;
    let resp = await req(`${host}/s/${encodeURIComponent(wd)}/p${p}.html`, {
        headers
    });
    return JSON.stringify({
        list: getList(resp.content),
...
async function play(flag, id, flags) {
    ...
    if (playerVarMatch) {
        ...
        if (playUrl) return JSON.stringify({
            parse: playUrl.includes('.m3u8') ? 0 : 1,
            url: playUrl,
            header: headers
        });
    }
