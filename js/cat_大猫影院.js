...
async function category(tid, pg, filter, extend) {
    ...
    let videos = json.list.map(item => ({
        vod_id: '/cqodetail/' + item.vod_id + '.html',
...
async function detail(id) {
    ...
    const playPageUrl = pdfh(thtml, '.p1&&.play a&&href');
    if (!playPageUrl) {
        return JSON.stringify({
            list: []
        });
    ...
    const lineInfo = lineBtnList.map(a => ({
        name: pdfh(a, 'a&&Text').replace(/\s+/g, '').replace(/(\D+)(\d+)/, '$1|共$2集'),
        href: pdfh(a, 'a&&href')
    }));
    const playUrlArr = [];
    ...
    return JSON.stringify({
        list: [{
            ...
            vod_year: (thtml.match(/<a href="\/cqosw\/[\s\S]*?-----------[\s\S]*?.html">([\s\S]*?)<\/a>/) || ["", ""])[1],
            vod_area: (thtml.match(/<a href="\/cqosw\/[\s\S]*?-[\s\S]*?----------.html">([\s\S]*?)<\/a>/) || ["", ""])[1],
            vod_remarks: (thtml.match(/状态：<\/span>([\s\S]*?)</) || ['', ''])[1],
            type_name: (thtml.match(/<a href="\/cqosw\/[\s\S]*?---[\s\S]*?--------.html" title="[\s\S]*?">([\s\S]*?)<\/a>/) || ["", ""])[1],
